...
public class ToursUtils {
    //создодим метод который будет сохранять один тур в 2хмерный массив
        ...
        for (int i = 0; i < tours.length; i++) {
            if (tours[i][0] != null && tours[i][1].equalsIgnoreCase(country)) {
                System.out.printf("Тур №%s в %s(%s), на %s, %s, %s, %s за %s \n"
                        , tours[i][0], tours[i][1], tours[i][2], tours[i][4],
                        tours[i][3], tours[i][6], tours[i][7], tours[i][5]);
            }
        }
    }
    ...
    public static void printToursByBudget(String[][] tours, int budget) {
        for (int i = 0; i < tours.length; i++) {
            ...
            if (tours[i][0] != null) {
                int spaceInex = tours[i][5].indexOf(" "); // узнали позицию пробела
                String priceStr = tours[i][5].substring(0, spaceInex); //вырезали только цену (без руб)
